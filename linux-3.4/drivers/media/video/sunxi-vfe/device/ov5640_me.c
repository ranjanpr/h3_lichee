...
#define APTINA_SENSOR
#define MT9V034_IDENT_SENSOR 0x1324
 ...
 */
#endif ///////////////////////PR-------////////////////////////////////////////
static int mt9v034_sensor_read(struct v4l2_subdev *sd, unsigned short reg,
    unsigned char *value)
{
        int ret=0;
        int cnt=0;
printk("\n**** inside mt9v034_sensor_read 8bit address and 16 bit data APTINA_SENSOR ****\n");

  ret=cci_read_a8_d16(sd,reg,value);
  while(ret!=0&&cnt<2)
  {
        ret = cci_read_a8_d16(sd,reg,value);
        cnt++;
  ...
  if(cnt>0)
        vfe_dev_dbg("MT9V034 sensor read retry=%d\n",cnt);
...
static int mt9v034_sensor_write(struct v4l2_subdev *sd, unsigned char reg,
    unsigned short value)
{
        int ret=0;
        int cnt=0;

  printk("\n**** inside mt9v034_sensor_write 8bit address and 16 bit data APTINA_SENSOR ****\n");

  ret = cci_write_a8_d16(sd,reg,value);
  while(ret!=0&&cnt<2)
  {
        ret = cci_write_a8_d16(sd,reg,value);
        cnt++;
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
  ...
  printk("\n*** Aptina is inide sensor_power on = %d\n", on);
...
static int sensor_detect(struct v4l2_subdev *sd)
{
  ...
  mt9v034_sensor_read(sd, 0x00, &rdval);

 printk("sensor version read value = %x\n",rdval);
   if(rdval != 0x1324)
       return -ENODEV;
   else
   printk("\n\nMT9V034 SENSOR FOUND ITS VESRION ID IS = %x\n",rdval);
   return 0;

#else

  unsigned char ddrdval;
...
#ifdef APTINA_SENSOR
printk("\n***** !!!!! here we neeed to add MT9V034 initialization to get the proper capture********\n");
	//reset MT9V034 BY WRITING 0X1 TO RESET REGISTER 0X0C
	ret = mt9v034_sensor_write(sd, 0x0c,0x0001);
 	 if(ret < 0) {
        ...
        udelay(10);
	//reset MT9V034 BY WRITING 0X1 TO RESET REGISTER 0X0C
	ret = mt9v034_sensor_write(sd, 0x0c,0x0000);
         if(ret < 0) {
	 vfe_dev_err("write sensor_default_regs error\n");
	 return ret;
	}
...
// Column Start
ret = mt9v034_sensor_write(sd, 0x01,0x0001);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
// Row Start
ret = mt9v034_sensor_write(sd, 0x02,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
// REG= 0x03, 480 Height
ret = mt9v034_sensor_write(sd, 0x03,0x01e0);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x04, 752 Width
ret = mt9v034_sensor_write(sd, 0x04,0x02f0);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

// REG= 0x05, 101 Horizontal Blanking default 94, 0x005E
ret = mt9v034_sensor_write(sd, 0x05,0x0065);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x06, 17 default 45, VERT_BLANK_CONTEXTA_REG
ret = mt9v034_sensor_write(sd, 0x06,0x0011);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x07, 392 default 904, CONTROL_MODE_REG
ret = mt9v034_sensor_write(sd, 0x07,0x0188);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x08, 0 COARSE_SHUTTER_WIDTH_1_CONTEXTA
ret = mt9v034_sensor_write(sd, 0x08,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x09, 0 COARSE_SHUTTER_WIDTH_2_CONTEXTA
ret = mt9v034_sensor_write(sd, 0x09,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x0A, 356 SHUTTER_WIDTH_CONTROL_CONTEXTA
ret = mt9v034_sensor_write(sd, 0x0a,0x0164);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x0B, 400  Shutter Time 0 - 523
ret = mt9v034_sensor_write(sd, 0x0b,0x0190);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x0D, 0 Read Mode Context A, 0x0300
ret = mt9v034_sensor_write(sd, 0x0d,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x0E, 0 Read Mode Context B, 0
ret = mt9v034_sensor_write(sd, 0x0e,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x0F, 256	 Sensor Type, HDR Enable, 0x0100
ret = mt9v034_sensor_write(sd, 0x0f,0x0100);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x1B, 0 LED out
ret = mt9v034_sensor_write(sd, 0x1b,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x1C, 770 DATA_COMPRESSION
ret = mt9v034_sensor_write(sd, 0x1c,0x0302);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x2C, 0 VREF_ADC_CONTROL, was 4, should be 0?
ret = mt9v034_sensor_write(sd, 0x2c,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x31, 39 was 31, V1_CONTROL_CONTEXTA
ret = mt9v034_sensor_write(sd, 0x31,0x0027);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
#if 0
//REG= 0x32, 26  V2_CONTROL_CONTEXTA
ret = mt9v034_sensor_write(sd, 0x32,0x1A);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x33, 5	was 18, V3_CONTROL_CONTEXTA
ret = mt9v034_sensor_write(sd, 0x33,0x0005);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x34, 3 V4_CONTROL_CONTEXTA
ret = mt9v034_sensor_write(sd, 0x34,0x0003);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x35, 64 Full gain
ret = mt9v034_sensor_write(sd, 0x35,0x0040);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x36, 16 	GLOBAL_GAIN_CONTEXTB_REG
ret = mt9v034_sensor_write(sd, 0x36,0x0010);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x39, 39 	was 37, V1_CONTROL_CONTEXTB
ret = mt9v034_sensor_write(sd, 0x39,0x0027);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x3A, 38 			//was 32, V2_CONTROL_CONTEXTB
ret = mt9v034_sensor_write(sd, 0x3a,0x0026);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x3B, 5 was 3, V3_CONTROL_CONTEXTB
ret = mt9v034_sensor_write(sd, 0x3b,0x0005);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x3C, 3 V4_CONTROL_CONTEXTB
ret = mt9v034_sensor_write(sd, 0x3c,0x0003);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x46, 8989 DARK_AVG_THRESHOLDS
ret = mt9v034_sensor_write(sd, 0x46,0x231d);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x47, 128 CALIB_CONTROL_REG
ret = mt9v034_sensor_write(sd, 0x47,0x0080);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x48, 0 Black Level Calibration Value
ret = mt9v034_sensor_write(sd, 0x48,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x4C, 2 STEP_SIZE_AVG_MODE
ret = mt9v034_sensor_write(sd, 0x4c,0x0002);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x80 to 0x98, 4 Gain mandatory against white box
ret = mt9v034_sensor_write(sd, 0x80,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x81,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x82,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x83,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x84,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x85,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x86,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x87,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x88,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x89,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x8a,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x8b,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x8c,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x8d,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x8e,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x8f,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x90,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
ret = mt9v034_sensor_write(sd, 0x91,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x92,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x93,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x94,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x95,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
ret = mt9v034_sensor_write(sd, 0x96,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x97,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

ret = mt9v034_sensor_write(sd, 0x98,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0x99, 0 	X0_SLASH5
ret = mt9v034_sensor_write(sd, 0x99,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
#if 0
//REG= 0x9A, 154 			//was 150, X1_SLASH5
ret = mt9v034_sensor_write(sd, 0x9a,0x009a);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x9B, 300 X2_SLASH5
ret = mt9v034_sensor_write(sd, 0x9b,0x012c);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x9C, 450 X3_SLASH5
ret = mt9v034_sensor_write(sd, 0x9c,0x01c2);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x9D, 600 			// X4_SLASH5
ret = mt9v034_sensor_write(sd, 0x9d,0x0258);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x9E, 752 X5_SLASH5
ret = mt9v034_sensor_write(sd, 0x9e,0x02f0);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0x9F, 0 Y0_SLASH5
ret = mt9v034_sensor_write(sd, 0x9f,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xA0, 96 Y1_SLASH5
ret = mt9v034_sensor_write(sd, 0xa0,0x0060);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xA1, 192 Y2_SLASH5
ret = mt9v034_sensor_write(sd, 0xa1,0x00c0);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xA2, 288 Y3_SLASH5
ret = mt9v034_sensor_write(sd, 0xa2,0x0120);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xA3, 384 Y4_SLASH5
ret = mt9v034_sensor_write(sd, 0xa3,0x0180);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xA4, 480 Y5_SLASH5
ret = mt9v034_sensor_write(sd, 0xa4,0x01e0);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xA5, 58 DESIRED_BIN
ret = mt9v034_sensor_write(sd, 0xa5,0x003a);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xA6, 2 			// EXP_SKIP_FRM_H
ret = mt9v034_sensor_write(sd, 0xa6,0x0002);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
#if 0
//REG= 0xA8, 0 EXP_LPF
ret = mt9v034_sensor_write(sd, 0xa8,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xA9, 2 GAIN_SKIP_FRM
ret = mt9v034_sensor_write(sd, 0xa9,0x0002);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xAA, 2 GAIN_LPF_H
ret = mt9v034_sensor_write(sd, 0xaa,0x0002);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xAB, 64 MAX_GAIN
ret = mt9v034_sensor_write(sd, 0x64,0x0040);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
#if 0
//REG= 0xAC, 1 MIN_COARSE_EXPOSURE
ret = mt9v034_sensor_write(sd, 0xac,0x0001);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xAD, 480 MAX_COARSE_EXPOSURE
ret = mt9v034_sensor_write(sd, 0xad,0x01e0);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xAE, 20 			// BIN_DIFF_THRESHOLD
ret = mt9v034_sensor_write(sd, 0xae,0x0014);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
#if 0
//REG= 0xAF, 0 			// AUTO_BLOCK_CONTROL
ret = mt9v034_sensor_write(sd, 0xaf,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
#endif
//REG= 0xB0, 44000 		// PIXEL_COUNT
ret = mt9v034_sensor_write(sd, 0xb0,0xabe0);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xB1, 2 LVDS_MASTER_CONTROL
ret = mt9v034_sensor_write(sd, 0xb1,0x0002);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xB2, 16 	LVDS_SHFT_CLK_CONTROL
ret = mt9v034_sensor_write(sd, 0xb2,0x0010);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xB3, 16 	LVDS_DATA_CONTROL
ret = mt9v034_sensor_write(sd, 0xb3,0x0010);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xB4, 0 LVDS_DATA_STREAM_LATENCY
ret = mt9v034_sensor_write(sd, 0xb4,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xB5, 0 LVDS_INTERNAL_SYNC
ret = mt9v034_sensor_write(sd, 0xb5,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xB6, 0 LVDS_USE_10BIT_PIXELS
ret = mt9v034_sensor_write(sd, 0xb6,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xB7, 0 			// STEREO_ERROR_CONTROL
ret = mt9v034_sensor_write(sd, 0xb7,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xBF, 22 INTERLACE_FIELD_VBLANK
 ret = mt9v034_sensor_write(sd, 0xbf,0x0016);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xC0, 10 IMAGE_CAPTURE_NUM
ret = mt9v034_sensor_write(sd, 0xc0,0x000a);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0xC2, 0default is 2112, but it makes a bad image, ANALOG_CONTROLS
ret = mt9v034_sensor_write(sd, 0xc2,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xC6, 0 NTSC_FV_CONTROL
ret = mt9v034_sensor_write(sd, 0xc6,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xC7, 17430 NTSC_HBLANK
ret = mt9v034_sensor_write(sd, 0xc7,0x4416);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xC8, 17441 NTSC_VBLANK
ret = mt9v034_sensor_write(sd, 0xc8,0x4421);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xC9, 1 was 2, COL_WINDOW_START_CONTEXTB_REG
ret = mt9v034_sensor_write(sd, 0xc9,0x0001);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xCA, 4 ROW_WINDOW_START_CONTEXTB_REG
ret = mt9v034_sensor_write(sd, 0xca,0x0004);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xCB, 480 ROW_WINDOW_SIZE_CONTEXTB_REG
ret = mt9v034_sensor_write(sd, 0xcb,0x01e0);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xCC, 752 was 750, COL_WINDOW_SIZE_CONTEXTB_REG
ret = mt9v034_sensor_write(sd, 0xcc,0x02f0);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xCD, 94 was 256, HORZ_BLANK_CONTEXTB_REG
ret = mt9v034_sensor_write(sd, 0xcd,0x005e);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
#if 0
//REG= 0xCE, 45 was 256, VERT_BLANK_CONTEXTB_REG
ret = mt9v034_sensor_write(sd, 0xce,0x002d);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xCF, 478 was 400, COARSE_SHUTTER_WIDTH_1_CONTEXTB
ret = mt9v034_sensor_write(sd, 0xcf,0x01de);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xD0, 479 was 445 COARSE_SHUTTER_WIDTH_2_CONTEXTB
ret = mt9v034_sensor_write(sd, 0xd0,0x01df);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xD1, 100 SHUTTER_WIDTH_CONTROL_CONTEXTB
ret = mt9v034_sensor_write(sd, 0xd1,0x0064);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
#if 0
//REG= 0xD2, 480 was 450, COARSE_SHUTTER_WIDTH_TOTAL_CONTEXTB
ret = mt9v034_sensor_write(sd, 0xd2,0x01e0);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xD3, 0 FINE_SHUTTER_WIDTH_1_CONTEXTA
ret = mt9v034_sensor_write(sd, 0xd3,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xD4, 0 FINE_SHUTTER_WIDTH_2_CONTEXTA
ret = mt9v034_sensor_write(sd, 0xd4,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
#if 0
//REG= 0xD5, 0 FINE_SHUTTER_WIDTH_TOTAL_CONTEXTA
ret = mt9v034_sensor_write(sd, 0xd5,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xD6, 0 FINE_SHUTTER_WIDTH_1_CONTEXTB
ret = mt9v034_sensor_write(sd, 0xd6,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }

//REG= 0xD7, 0 FINE_SHUTTER_WIDTH_2_CONTEXTB
ret = mt9v034_sensor_write(sd, 0xd7,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xD8, 0 FINE_SHUTTER_WIDTH_TOTAL_CONTEXTB
ret = mt9v034_sensor_write(sd, 0xd8,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
// REG= 0xD9, 0 MONITOR_MODE_CONTROL
ret = mt9v034_sensor_write(sd, 0xd9,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xF0, 0 Bytewise Addr
ret = mt9v034_sensor_write(sd, 0xf0,0x0000);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
//REG= 0xFE, 48879		// Register Lock, 0xBEEF
ret = mt9v034_sensor_write(sd, 0xfe,0xbeef);
         if(ret < 0) {
         vfe_dev_err("write sensor_default_regs error\n");
         return ret;
        }
...
#ifdef APTINA_SENSOR
  return v4l2_chip_ident_i2c_client(client, chip, MT9V034_IDENT_SENSOR, 0);
#else
